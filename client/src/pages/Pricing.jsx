...
const Pricing = () => {
  ...
  useEffect(() => {
    const fetchUserSubscriptions = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            "Pricing: Raw subscription data received:",
            subscriptionData
          );

          // Extract subscriptions from the response object
          const subscriptions = subscriptionData.success
          ...
          console.log(
            "Pricing: Active subscriptions found:",
            activeSubscriptions
          );
    ...
    const handleSubscriptionUpdate = (event) => {
      console.log(
        "Pricing: Subscription updated event received:",
        event.detail
      );
  ...
  const isCurrentPlan = (planTitle) => {
    if (!user || !accessControl.currentPlan) {
      console.log("isCurrentPlan: No user or currentPlan", {
        user: !!user,
        currentPlan: accessControl.currentPlan,
      });
      return false;
  ...
  const getUpgradeSuggestion = (plan) => {
    ...
    if (
      ...
      accessControl.currentPlan === "STARTER"
    ) {
    ...
    if (planType === "SOLO") {
      if (accessControl.currentPlan === "SOLO") {
        ...
        if (availableModules.length > 0) {
          return `${purchasedModules.length} module${
            purchasedModules.length > 1 ? "s" : ""
          } purchased. ${availableModules.length} more available`;
        } else {
          ...
          {plans.map((plan, idx) => (
            ...
            >
              ...
              <p className="text-4xl font-extrabold text-[#042038] mt-1">
                {plan.title === "PRO PLAN" &&
                accessControl.currentPlan === "SOLO" &&
                accessControl.soloModules.length > 0 ? (
                  <span>
                    ...
                    <span className="text-green-600">
                      ₹{accessControl.calculateUpgradePrice("PRO").totalPrice}
                    </span>
                  </span>
                ) : (
                  plan.price
              ...
              {plan.title === "PRO PLAN" &&
                ...
                accessControl.soloModules.length > 0 && (
                  <p className="text-xs text-green-600 font-semibold mt-1">
                    Save ₹
                    {accessControl.calculateUpgradePrice("PRO").soloDiscount}{" "}
                    from your SOLO plans!
                  </p>
                )}
              ...
              {(isCurrentPlan(plan.title) ||
                ...
                getUpgradeSuggestion(plan) && (
                  <div className="mt-3 text-center">
                    ...
                    {plan.title === "SOLO PLAN" &&
                    accessControl.currentPlan === "SOLO" &&
                    accessControl.getAvailableForPurchase().length === 0 ? (
                      <button
